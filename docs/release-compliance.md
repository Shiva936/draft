...
| Submit finalization | Implemented | Canonical state uses submit-named config, hooks, events, signed receipts, and lifecycle records. Superseded save-named fields and commands are not accepted. |
...
| Draft Console | Implemented | The loopback-only `console/` gateway embeds committed `console/dist/` assets at compile time; `console/web/` supports type-checking and deterministic production builds. CI rebuilds the assets and rejects a dirty diff. |
| Extension boundary | Implemented | The unversioned extension-package contract uses literal schema v1, content hashes, atomic local installation, explicit enabled state, and typed trust provenance; Draft does not execute extension entrypoints or ship protocol bridges. |
...
| Public documentation | Implemented | README, guides, reference, internals, changelog, release notes, roadmap, and this matrix describe the command and safety boundary. |
