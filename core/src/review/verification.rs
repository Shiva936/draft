...
/// Strict project verification command configuration (`verify.toml`).
...
pub struct VerificationCheck {
    pub name: String,
    pub command: String,
    pub risk: crate::review::risk::RiskLevel,
    #[serde(default)]
...
impl crate::contracts::VersionedContract for ProjectStateReport {
    const CONTRACT: crate::contracts::ContractId = crate::contracts::ContractId::ProjectStateReport;
}
...
///
/// Policy failures are reported in the returned checks. Failures to read or
/// validate authoritative state retain their corruption/storage category.
pub fn verify_project_state(
    ...
    pack_verified: bool,
) -> DraftResult<ProjectStateReport> {
    let mut checks = Vec::new();

    let workspace_hash =
        crate::workspace::source_view::workspace_hash_cached(root, &paths.workspace_hash_cache())?;
    checks.push(ProjectStateCheck {
        name: "workspace_hash_computed".to_string(),
        passed: true,
        detail: workspace_hash.clone(),
    });

    let draft_touched: Vec<&String> = affected_paths
        .iter()
        .filter(|p| crate::support::pathguard::is_draft_path(p))
        .collect();
    checks.push(ProjectStateCheck {
    ...
    let ledger_ok = crate::trust::ledger::TrustLedger::open(root, workspace_id)?
        .verify_all()?
        .all_ok;
    checks.push(ProjectStateCheck {
    ...
    Ok(ProjectStateReport {
        schema_version: crate::contracts::current_version(
            crate::contracts::ContractId::ProjectStateReport,
        ),
        workspace_hash,
        checks,
        passed,
...
pub struct VerifyEvidence {
    ...
    /// Deterministic cache key for this exact immutable revision.
    ...
    pub fn validate_binding(&self, revision: &PackRevision) -> DraftResult<()> {
            ...
            || self.dependency_digests != revision.resolved_dependency_digests
        {
            return Err(DraftError::new(
                ...
                "verification evidence is bound to a different pack revision or dependencies",
            ));
        }
...
pub fn plan(input: &SelectionInput) -> VerifyEvidence {
    ...
    let mut evidence = VerifyEvidence {
        ...
        dependency_digests: input.dependency_digests.clone(),
        selected_tests,
