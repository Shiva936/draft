...
use crate::workspace::stable::SubmitMode;
...
pub(crate) enum SubmitHookPhase {
    Before,
    After,
}
...
impl SubmitHookConfig {
    fn entries(&self, phase: SubmitHookPhase) -> Vec<HookEntry> {
        match self {
            Self::Raw { command } if phase == SubmitHookPhase::Before => HookConfig::Raw {
...
pub(crate) struct ResolvedConfig {
    ...
    pub(crate) submit_message_template: String,
    pub(crate) pack_disposal: SubmitMode,
    hooks: HooksConfig,
}

impl ResolvedConfig {
    pub(crate) fn load(workspace: &Workspace) -> DraftResult<Self> {
        let global = crate::workspace::home::DraftGlobalStore::locate()?.config_toml();
        let config = resolve(
        ...
        Ok(Self {
            ...
            submit_message_template: config.submit.message_template,
            pack_disposal: SubmitMode::parse(&config.submit.pack_disposal)?,
            hooks: config.hooks,
        })
    }
    ...
    pub(crate) fn submit_hooks(&self, phase: SubmitHookPhase) -> Vec<HookEntry> {
        self.hooks
            .submit
            .as_ref()
...
fn submit_hook_config_command(hook: &SubmitHookConfig) -> String {
    match hook {
        ...
        SubmitHookConfig::Phases { before, after } => {
            format!(
                "{} before hook(s), {} after hook(s)",
                before.len(),
                after.len()
            )
        }
...
pub struct CandidateConfig {
    ...
    pub supports_patch_output: Option<bool>,
    pub supports_pack_output: Option<bool>,
...
pub struct ProtectedConfig {
    #[serde(default)]
    pub protected_files: Vec<ProtectedRuleConfig>,
}
...
impl DraftConfig {
    pub fn set(&mut self, key: &str, value: &str) -> DraftResult<()> {
        match key {
            ...
            "submit.pack_disposal" => {
                crate::workspace::stable::SubmitMode::parse(value)?;
                self.submit.pack_disposal = value.to_string();
            }
            "hooks.submit" => {
                self.hooks.submit = Some(SubmitHookConfig::Raw {
    ...
    pub fn unset(&mut self, key: &str) -> DraftResult<()> {
        match key {
            ...
            "submit.message_template" => self.submit.message_template.clear(),
            "submit.pack_disposal" => self.submit.pack_disposal = default_pack_disposal(),
            "hooks.submit" => self.hooks.submit = None,
...
pub fn config_hash(paths: &crate::workspace::layout::DraftLayout) -> DraftResult<String> {
...
fn encoded_config_value(key: &str, value: &str) -> DraftResult<Value> {
    let structured = match key {
        "hooks.submit" => Some(Value::try_from(SubmitHookConfig::Raw {
            command: value.to_string(),
        })),
