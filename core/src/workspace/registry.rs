...
impl ProjectRegistry {
        ...
        self.list()?
            ...
            .find(|entry| {
                entry.workspace_id == id_or_path
                    || canonical
                        .as_ref()
                        .is_some_and(|path| Path::new(&entry.repository_path) == path)
            })
            .ok_or_else(|| {
    ...
    pub fn upsert(
        ...
        if let Some(path_owner) = envelope.entries.iter().find(|entry| {
            entry.repository_path == canonical_text && entry.workspace_id != workspace_id
        }) {
        ...
        if let Some(existing) = envelope
            ...
            .find(|entry| entry.workspace_id == workspace_id)
        {
            if existing.repository_path != canonical_text {
                let old_path = existing.repository_path.clone();
            ...
            existing.last_stable_head = stable;
        ...
        let entry = ProjectRegistryEntry {
            ...
            name: canonical
                ...
                .unwrap_or("project")
                .into(),
            repository_path: canonical_text,
            ...
            last_workspace_revision: crate::workspace::source_view::WorkspaceRevision::derive(
                &canonical,
            )
            .ok()
    ...
    pub fn relocate(
        ...
        if envelope.entries.iter().any(|entry| {
            entry.repository_path == destination_text && entry.workspace_id != workspace_id
        }) {
    ...
    pub fn inspect(&self) -> DraftResult<Vec<RegistryIssue>> {
        ...
        for entry in entries {
            ...
            if !root.exists() {
                issues.push(issue(&entry, "path_missing", &entry.repository_path, true));
                continue;
            }
            if !Path::new(&entry.storage_path).is_dir() {
                ...
                match read_workspace_id(root) {
                    ...
                    Ok(actual) => {
                        if let Some(other) = live_ids.insert(actual, entry.repository_path.clone())
                        {
                            issues.push(issue(
                                &entry,
            ...
            if let Some(other) =
                paths.insert(entry.repository_path.clone(), entry.workspace_id.clone())
            {
                issues.push(issue(
                    &entry,
...
fn read_workspace_id(root: &Path) -> DraftResult<String> {
    let metadata: crate::workspace::WorkspaceMetadata =
        crate::contracts::read_persisted(&root.join(".draft/workspace.json"))?;
...
mod tests {
    ...
    fn duplicate_registration_is_idempotent_and_relocation_is_explicit() {
        ...
        std::env::set_var("DRAFT_GLOBAL_HOME", global.path().join("home"));
        let workspace_id = crate::support::common::WorkspaceId::generate().to_string();
        let layout = crate::workspace::layout::DraftLayout::for_root(project.path());
        ...
        crate::support::fsutil::write_json(
            ...
            &crate::workspace::WorkspaceMetadata {
                ...
                workspace_id: crate::support::common::WorkspaceId::new(workspace_id.clone()),
                draft_version: crate::DRAFT_VERSION.into(),
                created_at: now(),
            },
        )
        ...
        let first = registry
            .upsert(&workspace_id, project.path(), None)
            .unwrap();
        let second = registry
            .upsert(&workspace_id, project.path(), None)
            .unwrap();
