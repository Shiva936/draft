...
use crate::trust::event::{EventKind, EventLog, EventRecord, NewEvent};
use crate::trust::identity::global::{self, ActorProfile};
use crate::trust::receipt::{self, ReceiptRecord, ReceiptStore, ReceiptVerification};
use crate::trust::signing::Keypair;
use crate::trust::transparency::TransparencyLog;
...
impl TrustLedger {
    ...
    pub fn open(root: &Path, workspace_id: &str) -> DraftResult<Self> {
        Self::open_at(root, workspace_id, DraftGlobalStore::locate()?)
    }

    /// Open the ledger against an explicit global store (used by tests and any
    /// caller that has already resolved the global home).
    pub fn open_at(root: &Path, workspace_id: &str, home: DraftGlobalStore) -> DraftResult<Self> {
    ...
    fn events(&self) -> EventLog {
        EventLog::workspace(self.paths.clone(), self.workspace_id.clone())
    }
    ...
    /// Record an action using a caller-reserved receipt identity. This is used
    /// when a public operation report must return the receipt id before the
    /// canonical event is committed; the persisted record is still the sole
    /// signed [`ReceiptRecord`] contract.
    pub fn record_with_receipt_id(
        &self,
        kind: EventKind,
        subject_id: Option<String>,
        candidate_id: Option<String>,
    ...
    fn revoked_keys(&self) -> DraftResult<Vec<String>> {
    ...
    fn resolve_transparency_key(
        &self,
        entry: &crate::trust::transparency::TransparencyEntry,
...
mod tests {
    ...
    fn record_produces_verifiable_ledger() {
        ...
        let home = DraftGlobalStore::at(tmp.path().join("global/.draft"));
    ...
    fn tampered_receipt_detected_by_verify_all() {
        let tmp = tempfile::tempdir().unwrap();
        let home = DraftGlobalStore::at(tmp.path().join("g/.draft"));
        ...
        let rpath = DraftLayout::for_root(&root).receipt_file(&out.receipt.receipt_id);
        let mut r: ReceiptRecord = crate::contracts::read_persisted(&rpath).unwrap();
        r.workspace_hash = "sha256:tampered".into();
        crate::support::fsutil::write_json(&rpath, &r).unwrap();
