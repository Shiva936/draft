...
impl crate::contracts::VersionedContract for SigningKeyRecord {
    const CONTRACT: crate::contracts::ContractId = crate::contracts::ContractId::SigningKeyRecord;
}
...
impl Keypair {
    ...
    pub fn save(&self, path: &Path) -> DraftResult<()> {
        if let Some(parent) = path.parent() {
            ...
            let _ = crate::support::hidden::restrict_dir(parent, 0o700);
        }
        let record = SigningKeyRecord {
            schema_version: crate::contracts::current_version(
                crate::contracts::ContractId::SigningKeyRecord,
            ),
            algorithm: SIGNATURE_ALGORITHM.into(),
            key_material: B64.encode(self.inner.to_bytes()),
        };
        crate::support::fsutil::write_json(path, &record)
            .map_err(|e| DraftError::storage(format!("write signing key: {e}")))?;
        crate::support::hidden::restrict_file(path, 0o600)
    ...
    pub fn load(path: &Path) -> DraftResult<Self> {
        let record: SigningKeyRecord = crate::contracts::read_persisted(path)?;
        if record.algorithm != SIGNATURE_ALGORITHM {
            return Err(DraftError::new(
                crate::support::error::DraftErrorKind::CorruptData,
                "signing key algorithm is invalid",
            ));
        }
        let bytes = B64.decode(record.key_material).map_err(|_| {
            DraftError::new(
                crate::support::error::DraftErrorKind::CorruptData,
                "signing key is not valid base64",
            )
        })?;
