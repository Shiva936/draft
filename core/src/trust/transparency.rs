...
impl crate::contracts::VersionedContract for TransparencyEntry {
    const CONTRACT: crate::contracts::ContractId = crate::contracts::ContractId::TransparencyEntry;
}
...
impl TransparencyLog {
    ...
    pub fn read_all(&self) -> DraftResult<Vec<TransparencyEntry>> {
        ...
        for (i, line) in text.lines().enumerate() {
            ...
            let entry: TransparencyEntry = crate::contracts::decode_persisted(line.as_bytes())
                .map_err(|error| error.with_context(format!("transparency line {}", i + 1)))?;
            out.push(entry);
        }
        Ok(out)
    }
    ...
    pub fn append(
        ...
        let mut entry = TransparencyEntry {
            schema_version: crate::contracts::current_version(
                crate::contracts::ContractId::TransparencyEntry,
            ),
        ...
        F: Fn(&TransparencyEntry) -> Option<String>,
    {
        ...
        for (i, entry) in all.iter().enumerate() {
            ...
            let sig_ok = crate::trust::signing::verify_b64(
                &pk,
...
fn corrupt(i: usize, msg: &str) -> DraftError {
    DraftError::new(
        crate::support::error::DraftErrorKind::OperationLogCorrupt,
...
mod tests {
    ...
    fn append_and_verify_chain() {
        ...
        let log = TransparencyLog::new(DraftLayout::for_root(tmp.path()));
