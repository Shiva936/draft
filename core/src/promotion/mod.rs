//! Promotion: accepting a ChangePack's work into the project's Baseline.
//!
//! # Why this is journalled
//!
//! A promotion is several durable effects that must agree: the control state
//! advances to a new accepted Baseline, the ChangePack becomes `Completed`, a
...
//! the planned ChangePack completion* before anything moves, and recovery reads
//! the answer off the two authoritative records rather than inferring it.
//!
//! # Why two records are consulted
//!
//! The commit point advances `ProjectControlState`; the ChangePack completion
//! follows under the ChangePack's own lock. Between them a crash leaves the
//! control state moved and the ChangePack still `Active` — a real, legal,
...
pub use journal::{
    classify, resolve, ChangePackMatch, ControlMatch, PromotionJournalState, PromotionResolution,
};
