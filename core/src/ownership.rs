...
pub fn evaluate(
    ...
    let Some(owners) = value.get("owners").and_then(toml::Value::as_table) else {
        return Ok(OwnershipReport::empty());
    };
    let mut matches = Vec::new();
    let mut domains = BTreeSet::new();
    for path in paths {
        for (pattern, raw) in owners {
            let owner_list = owner_values(raw);
...
fn owner_values(raw: &toml::Value) -> Vec<String> {
    match raw {
