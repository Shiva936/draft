//! Content-addressed project objects and rebuildable compact-segment indexes.
...
pub(crate) struct ObjectSegmentIndex {
    pub(crate) schema_version: u32,
    pub(crate) objects: BTreeMap<String, String>,
}

impl crate::contracts::VersionedContract for ObjectSegmentIndex {
    const CONTRACT: crate::contracts::ContractId = crate::contracts::ContractId::ObjectSegmentIndex;
}

impl Default for ObjectSegmentIndex {
    fn default() -> Self {
        Self {
            schema_version: crate::contracts::current_version(
                crate::contracts::ContractId::ObjectSegmentIndex,
            ),
...
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub(crate) struct ObjectSegment {
    pub(crate) schema_version: u32,
    pub(crate) id: String,
    pub(crate) created_at: Timestamp,
    pub(crate) entries: Vec<ObjectSegmentEntry>,
}

impl crate::contracts::VersionedContract for ObjectSegment {
    const CONTRACT: crate::contracts::ContractId = crate::contracts::ContractId::ObjectSegment;
}

#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub(crate) struct ObjectSegmentEntry {
...
impl ObjectStore {
    pub fn new(layout: DraftLayout) -> Self {
        Self { layout }
    }
    ...
    fn get_packed_bytes(&self, object_ref: &str) -> DraftResult<Vec<u8>> {
        let index = read_object_segment_index(&self.layout)?;
        let change_name = index.objects.get(object_ref).ok_or_else(|| {
            DraftError::new(
                DraftErrorKind::CorruptData,
                format!("object {object_ref} is missing from loose and compact storage"),
            )
        })?;
        let segment_path = self.layout.object_segments_dir().join(change_name);
        let segment_bytes = fs::read(&segment_path)?;
        let json = zstd::stream::decode_all(segment_bytes.as_slice()).map_err(|error| {
            DraftError::new(
                DraftErrorKind::CorruptData,
                format!(
                    "object segment decompression failed for {}: {error}",
                    segment_path.display()
                ),
            )
        })?;
        let segment: ObjectSegment = crate::contracts::decode_persisted(&json)?;
        let entry = segment
            .entries
            .into_iter()
            ...
            .ok_or_else(|| {
                DraftError::new(
                    DraftErrorKind::CorruptData,
                    format!("object {object_ref} is absent from indexed segment {change_name}"),
                )
            })?;
...
pub(crate) fn read_object_segment_index(layout: &DraftLayout) -> DraftResult<ObjectSegmentIndex> {
    let path = layout.object_segments_dir().join("index.json");
    if !path.exists() {
        return Ok(ObjectSegmentIndex::default());
    }
    crate::contracts::read_persisted(&path)
}

pub(crate) fn write_object_segment_index(
    layout: &DraftLayout,
    index: &ObjectSegmentIndex,
) -> DraftResult<()> {
    write_json(&layout.object_segments_dir().join("index.json"), index)
}
