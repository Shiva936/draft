...
/**
 * The ChangePacks view of Work: every ChangePack with a sealed revision, and what has
  ...
  return (
    <>
      ...
      <QueryState
        ...
        empty={
          <Panel>
            <EmptyState label="This project has no ChangePacks yet." />
          </Panel>
        }
      >
        {(view: GraphView) => {
          const open = view.change_packs.filter((change) => change.revisions.length > 0);
          if (open.length === 0) {
            return (
              <Panel>
                <PanelHeader
                ...
                />
                <EmptyState label="No ChangePack has a sealed revision to authorize." />
              </Panel>
            );
          }
          return (
...
function ChangePanel({
  ...
  workspaceId: string;
  base: string;
  change: ChangePackView;
  baseline: BaselineView | null;
  onChanged: () => void;
}) {
  const revision = change.revisions[0];
  const query = useQuery({
    queryKey: ["graph-authorization", workspaceId, change.change_pack, revision.id],
    queryFn: () =>
      api<AuthorizationView>(
        `${base}/authorization/${encodeURIComponent(change.change_pack)}/${encodeURIComponent(revision.id)}`,
      ),
  });
  ...
  const promote = useMutation({
    mutationFn: (view: AuthorizationView) => {
      ...
      return mutate<unknown>(`${base}/promote`, {
        change_pack_id: view.change_pack,
        revision_pack_id: view.revision_pack,
      ...
      {(view: AuthorizationView) => {
        ...
        return (
          <Panel>
            <PanelHeader
              title={
                // Into the ChangePack's own scope, where §8.3's fourteen views —
                // intent, scope, impact, recovery and the rest — live.
                <Link to={encodeURIComponent(change.change_pack)}>
                  {change.change_pack} · {view.revision_pack}
                </Link>
              }
